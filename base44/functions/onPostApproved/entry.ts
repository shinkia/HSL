...
Deno.serve(async (req) => {
  try {
    ...
    if (data?.status !== 'published') return Response.json({ skipped: true });
    if (old_data?.status === 'published') return Response.json({ skipped: true });
    ...
    if (users[0].first_post_approved) return Response.json({ skipped: true });
